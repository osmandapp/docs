...
|  **{% data variables.android-values.recalculate_route_in_deviation}** (The route will be recalculated if the distanc from the route to the current location is more then selected value)  |   `turn on/off`  | Android/iOS  |   
...
|   | ✔   |   |   ||  
| **{% data variables.ios-values.road_speeds%}**   | set the min and max travel speed for a roads on the route  |   iOS |
|   | ✔  |   |   |   |   
