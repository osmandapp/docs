...
| [{% data variables.ios-values.change_map_source %}](/osmand/map/raster-maps) | Create a list of map sources and cycle through them.  <br> - **Show an interim dialog**: displays dialog with maps list <br> - **Do not show an interim dialog**: map sources will be changed by defined  order. <br> - **Note**: you could have 1 action with multiple map sources or many actions with 1 map source. |
